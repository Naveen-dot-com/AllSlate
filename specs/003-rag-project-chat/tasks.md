...
- [X] T005 Create `Message`/`MessageCitation`/`Conversation` models (dataclass-based, matching existing repo style) in backend/app/models/message.py and backend/app/models/conversation.py
- [X] T006 Implement project/thread ownership validation and shared FastAPI dependencies in backend/app/api/dependencies.py (`get_owned_conversation`) and backend/app/api/routes/chat.py
- [ ] T007 Add database migration for `messages` and `message_citations` tables with RLS in backend/migrations/003_rag_project_chat.sql (deferred — no Supabase/Postgres migration tooling wired up in this repo yet; current persistence is in-memory)
- [X] T008 Implement project-scoped retrieval and relevance filtering in backend/app/rag/retriever.py
- [ ] T009 [P] Implement shared error handling, logging, and SSE event serialization in backend/app/core/errors.py and backend/app/core/sse.py (SSE serialization currently inlined in backend/app/api/routes/chat.py; not yet extracted to a shared core module)
- [X] T010 [P] Create backend service layer for message creation, history fetch, and ordered inserts in backend/app/services/message_service.py and backend/app/services/conversation_service.py
...
- [X] T011 [P] [US1] Contract test for POST /api/v1/projects/{project_id}/conversations/{conversation_id}/ask in backend/tests/contract/test_chat_contract.py
- [X] T012 [P] [US1] Integration test for grounded answer + insufficient-evidence paths in backend/tests/integration/test_rag_chat_ask.py
- [X] T013 [P] [US1] Unit test for project-scoped retrieval and citation assembly in backend/tests/unit/test_retriever.py and backend/tests/unit/test_generation.py

### Implementation for User Story 1

- [X] T014 [P] [US1] Implement document context assembly and citation indexing in backend/app/rag/context_assembly.py
- [X] T015 [US1] Implement citation mapping and asset-reference resolution in backend/app/rag/citations.py
- [X] T016 [US1] Implement ask-question SSE stream, phase transitions, and answer persistence in backend/app/api/routes/chat.py
- [X] T017 [US1] Add retrieval + generation orchestration logic and insufficient-evidence short-circuit in backend/app/rag/generation.py and backend/app/rag/graph.py
- [X] T018 [US1] Persist user and assistant messages with status lifecycle and citation rows in backend/app/services/message_service.py
- [X] T019 [US1] Add backend validation for empty questions, missing project/thread ownership, and failed generation states in backend/app/api/routes/chat.py
...
- [X] T026 [P] [US3] Integration/unit test for thread isolation (cross-project rejection) in backend/tests/unit/test_message_service.py and backend/tests/contract/test_chat_contract.py
- [ ] T027 [P] [US3] End-to-end test for multi-thread switching in frontend/tests/e2e/chat-threads.spec.ts (deferred — no frontend app yet)

### Implementation for User Story 3

- [X] T028 [P] [US3] Implement new conversation creation and history endpoints in backend/app/api/routes/chat.py
- [X] T029 [US3] Implement thread history retrieval service and ordered message serialization in backend/app/services/conversation_service.py and backend/app/services/message_service.py
...
- Backend for User Story 1 (grounded answers + citations + insufficient-evidence) and the
  thread-creation/history portion of User Story 3 are implemented and covered by unit,
  integration, and contract tests (36 tests passing across the repo).
- Persistence is currently in-memory (`ConversationService`/`MessageService`); the Supabase
  Postgres migration (T007) and Gemini/pgvector wiring remain stubbed behind swappable seams
  (`AnswerGenerator(llm_call=...)`, `ProjectScopedRetriever`) for future integration.
- Frontend (User Story 2's table/image rendering, User Story 3's thread-switch UI, User Story
  4's status indicators) is not implemented — this repository has no frontend application
  scaffolding (no package.json/Next.js app) prior to or after this session.
