...
- [ ] T004 Write migration adding `confidence` (`confident`/`partial`/`uncertain`, default
      ...
      `backend/migrations/` (deferred — no Supabase/Postgres migration tooling wired up in
      this repo yet; current persistence is in-memory, see `backend/app/models/document.py`)
- [ ] T005 Write migration extending `documents.status` CHECK constraint to allow
      ...
      `backend/migrations/` (deferred — same reason as T004; enum/field added directly to the
      in-memory `Document`/`ProcessingStatus` model instead)
- [X] T006 [P] Update the `Element` (`ProcessedElement`) model with `confidence`/
      `confidence_reason` fields in `backend/app/models/document.py` (no separate
      `element.py` file exists in this repo; `ProcessedElement` lives alongside `Document`)
- [X] T007 [P] Update the `Document` model with `stored_partial` status value and
...
- [X] T018 [US1] Extend `backend/app/pipeline/nodes/ocr.py` to compute per-page confidence via
      the bucketing utility (T008) and attach `confidence`/`confidence_reason` to OCR output
      (depends on T008)
- [X] T019 [US1] Extend `backend/app/pipeline/nodes/partition.py` to compute per-element
...
- [X] T020 [US1] Extend `backend/app/pipeline/nodes/partition.py` (or a dedicated language
...
- [X] T021 [US1] Add explicit conditional error edges from the `ocr` and `partition` nodes to
...
- [X] T022 [US1] Wire document-level status rollup (T010) into the pipeline's final stage so a
...
- [X] T023 [US1] Ensure `processing_status_events` records a status transition for
...
- [X] T024 [P] [US2] Contract test: `GET .../documents/{document_id}` response includes
...
- [X] T025 [P] [US2] Contract test: `GET .../documents/{document_id}/elements` response
      includes `confidence`/`confidence_reason` per element in
      `backend/tests/contract/test_document_elements.py`
- [X] T026 [P] [US2] Integration test: a document with mixed confident/partial elements shows
...
- [X] T027 [P] [US2] Integration test: a fully confident document shows no partial/uncertain
      indicators anywhere (no false positives), in
      `backend/tests/integration/test_hardening_no_false_positives.py`

### Implementation for User Story 2

- [X] T028 [US2] Update the document detail service to compute and return
      `has_low_confidence_content` and `failure_category` in
      `backend/app/services/document_status.py` (depends on T007, T010)
- [X] T029 [US2] Update the document detail API route to serialize the extended fields in
...
- [X] T034 [P] [US3] Unit test: `errors.py` mapping (T009) produces distinct
...
- [X] T035 [P] [US3] Integration test: three fixtures designed to fail for three different
...
- [X] T036 [US3] Refine `backend/app/pipeline/nodes/errors.py` reason templates to reference
...
- [X] T037 [US3] Ensure `failure_reason` and `failure_category` are both persisted together on
...
- [X] T038 [P] Integration test: reprocessing the same hardening fixture 3x under unchanged
...
- [X] T039 [P] Integration test: a deliberately malformed/corrupted fixture triggers a
...
- [X] T040 Ensure retrieval/citation code in `backend/app/rag/citations.py`
...
- [X] T041 Run [quickstart.md](./quickstart.md) end-to-end validation across all 5 scenarios
      and confirm all success criteria (SC-001–SC-006) are met (validated via the automated
      hardening test suite — `backend/tests/unit/` and `backend/tests/integration/` — since no
      live worker process or real fixture PDF binaries exist in this repo yet; see
      `backend/tests/fixtures/hardening/README.md`)
